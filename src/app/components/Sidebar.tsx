...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed right-0 lg:left-0 lg:right-auto top-20 h-[calc(100vh-5rem)] bg-background border-l lg:border-r border-border z-40 transition-transform duration-300',
          'w-64 flex flex-col',
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <div className="lg:hidden space-y-2 mb-6">
            ...
            <Link
              to="/"
              onClick={onClose}
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-muted-foreground hover:text-green-400 hover:bg-card transition-all"
            >
            ...
            <div className="h-px bg-accent mx-4 my-4" />
          </div>

          <p className="hidden lg:block text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-4 mb-2">Dashboard</p>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-green-500 text-primary-foreground shadow-lg shadow-green-500/20 font-bold'
                    : 'text-muted-foreground hover:text-green-400 hover:bg-card border border-transparent hover:border-green-500/20'
                )}
              >
