...
export default function FarmDetailPage() {
  ...
  const handleUpdateDevice = async (deviceId: number, newName: string, isActive: boolean) => {
    ...
    } catch (error) { alert("Device update failed"); }
  };
  ...
  const handleSaveField = async (fieldData: any) => {
    ...
    } catch (error) { alert("Field save failed."); }
  };
  ...
  if (isLoading) return <div className="flex h-screen items-center justify-center bg-[#0a0a0a]"><Loader2 className="w-12 h-12 text-[#48D87D] animate-spin" /></div>;
  if (!farm) return <div className="p-20 text-center"><h2 className="text-white text-2xl font-bold">Node not found</h2></div>;

  return (
    <div className="p-4 lg:p-8 pt-6 bg-[#0a0a0a] min-h-screen text-slate-100">
      <div className="max-w-7xl mx-auto">
        <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              ...
              <h1 className="text-4xl font-black text-white uppercase tracking-tighter italic">{farm.farmName}</h1>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
             ...
             <Card className="bg-[#111] border-slate-800 shadow-2xl p-6">
                <h3 className="text-white font-black text-xs uppercase tracking-widest flex items-center gap-2 mb-6 italic">
                  <Leaf size={14} className="text-[#48D87D]" /> Live Soil Nutrients (NPK)
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                   <div className="space-y-1">
                     <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Nitrogen (N)</p>
                   ...
                   <div className="space-y-1">
                     <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Phosphorus (P)</p>
                   ...
                   <div className="space-y-1">
                     <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Potassium (K)</p>
             ...
             <Card className="bg-[#111] border-slate-800 shadow-2xl p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-white font-black text-xs uppercase tracking-widest flex items-center gap-2 italic">
             ...
             <Card className="bg-[#111] border-slate-800 overflow-hidden shadow-2xl h-[400px]">
                <FarmMap farms={[farm]} />
             </Card>
          </div>

          <div className="space-y-8">
            <Card className="bg-[#111] border-slate-800 shadow-2xl">
                ...
                <div className="space-y-6">
                  {farm.fields?.map((field: any) => (
                    <div 
                      ...
                      className={`bg-gradient-to-b from-[#1a1a1a] to-[#111] p-4 rounded-xl border group relative cursor-pointer transition-all ${selectedFieldId === field.id ? 'border-[#48D87D]' : 'border-slate-800'}`}
                    >
                      ...
                      <div className="mt-4 space-y-2 border-t border-slate-800/50 pt-4">
                        ...
                        {devices[field.id]?.map((dev) => {
                          ...
                          return (
                            <div key={dev.id} className="flex items-center justify-between bg-black/40 p-2.5 rounded-lg border border-white/5 group/dev hover:border-[#48D87D]/30 transition-all">
                        ...
                        <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setSelectedFieldId(field.id); setIsDeviceModalOpen(true); }} className="w-full h-8 mt-2 border border-dashed border-slate-800 text-slate-500 hover:text-[#48D87D] text-[9px] font-bold uppercase tracking-widest transition-all">
...
function StatusCard({ icon: Icon, title, value, unit, color }: any) {
  return (
    <Card className="bg-[#111] border-slate-800 relative group transition-all hover:bg-black shadow-lg">
